...
def node_summary_from_num(interface: Any, node_num: int) -> dict[str, Any]:
    ...
    if not summary.get("id") and hasattr(interface, "_nodeNumToId"):
        try:
            summary["id"] = interface._nodeNumToId(node_num, False) or f"{node_num:08x}"
        except Exception:
            summary["id"] = f"{node_num:08x}"
    return summary
...
class SQLiteStore:
    ...
    def snapshot(self, mesh_host: str, max_traces: int) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        ...
        for row in reversed(trace_rows):
            ...
            towards_nums = [int(value) for value in towards_raw if isinstance(value, (int, float, str)) and str(value).lstrip("-").isdigit()]
            back_nums = [int(value) for value in back_raw if isinstance(value, (int, float, str)) and str(value).lstrip("-").isdigit()]
            traces.append(
                {
...
def resolve_mesh_partition_key(interface: Any, fallback_host: str) -> str:
    ...
    if local_num is not None:
        try:
            ...
            node_id = None
            if hasattr(interface, "_nodeNumToId"):
                try:
                    node_id = interface._nodeNumToId(node_num, False)
                except Exception:
                    node_id = None
            if node_id:
...
def main() -> int:
    ...
    map_server: ThreadingHTTPServer | None = None
    ...
    try:
        ...
        map_state = MapState(store=store, mesh_host=partition_key, max_traces=800, log_buffer=log_buffer)
        emit(
